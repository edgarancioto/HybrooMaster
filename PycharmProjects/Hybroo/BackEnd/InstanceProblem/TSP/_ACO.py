...
class ACO(object):
    ...
    def solve(self, instance):
        ...
        for gen in range(self.generations):
            ants = [_Ant(self, self.instance_problem) for _ in range(self.ant_count)]
            for ant in ants:
                for i in range(self.instance_problem.dimension - 1):
                    ant.select_next()
                ant.total_cost = self.instance_problem.compute_tsp_distance(ant.cities)
                if ant.total_cost < self.best_cost:
                    self.best_cost = ant.total_cost
                    self.best_solution = [] + ant.cities
                ant.update_pheromone_delta()
            self.fitness_list.append(self.best_cost)
            self._update_pheromone(ants)
        return self.best_solution, self.best_cost, self.fitness_list
